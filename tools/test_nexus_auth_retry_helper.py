...
def main():
    ...
    ck("communication-survival-policy", "COMMUNICATION_SURVIVAL_POLICY.json" in registry and "A+B+C" in continuity and ("25-minute" in continuity or "25 minutes" in continuity))
    ...
    ck("communication-protocol-ref", '"schema": "bcp.communication_protocol/3"' in comm_protocol and '"normal_close_owner": "PRIMARY_ASSISTANT"' in comm_protocol and '"scheduled_backup_required": false' in comm_protocol)
    ck("communication-state-machine", '"schema": "bcp.communication_state_machine/3"' in comm_state and 'END_SEND_PENDING->END_ACKNOWLEDGED' in comm_state)
    ck("communication-delivery-key", '"delivery_key_required": true' in policy and '"delivery_key_required": true' in registry)
    ck("communication-backup-only", '"scheduled_close_guards_are_backup_only": true' in registry and '"scheduled_close_guards_required": false' in registry)
