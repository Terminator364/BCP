...
def main():
    ...
    ck("normal-closeout-no-automation-dependency", '"two_guard_closeout_required": false' in policy)
    ck("normal-closeout-23-min", '"normal_closeout_offset_minutes": 23' in policy)
    ck("backup-close-24-min", '"backup_earliest_offset_minutes": 24' in policy)
    ck("hard-close-25-min", '"hard_close_guard_offset_minutes": 25' in policy)
    ...
    ck("continuation-start-end-email", '"email_start_notice_before_substantive_work": true' in registry and '"email_end_checkpoint_before_chat_pointer": true' in registry and ("START/END Gmail handshake" in continuity or "R77 Fresh Conversation Handoff" in continuity))
    ck("continuation-25-minute-cadence", '"cadence_target_minutes": 25' in registry and '"useful_work_target_minutes": 23' in registry and ("23 useful work" in continuity or "23 minutes" in continuity))
    ck("communication-survival-policy", "COMMUNICATION_SURVIVAL_POLICY.json" in registry and ("R77 Fresh Conversation Handoff" in continuity or "R74 KINLINK-Style Communication State-Machine Handoff" in continuity))
    ...
    print("R77_DELIVERY_STATE_MACHINE_AND_NEXUS_HELPER=PASS")
    return 0
